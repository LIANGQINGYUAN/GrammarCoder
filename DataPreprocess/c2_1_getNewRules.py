...
rules["comment_py -> End"] = len(rules)
rules["string_literal_py -> End"] = len(rules)
rules["start -> python"] = len(rules)
rules['line_continuation_py -> End'] = len(rules)
...
def mytree2tokens(datas):
    ...
    for i in tqdm(range(len(datas))):
        ...
        tokendatas.append({'rulelist': [rules['start -> python']]+ rulelist, 'id': datas[i]['id']})
    return tokendatas, rules
...
def mytree2tokens_json(rfile, wfile):
    ...
    for i in tqdm(range(len(datas))):
        ...
        getRule(root, 0)
    pickle.dump(newrulelist, open(wfile, 'wb'))
